...
def main(cram, intervals):
    ...
    dxpy.download_dxfile(cram.get_id(), "sample.cram")
    dxpy.download_dxfile(index.get_id(), "sample.cram.crai")
    ...
    for interval_file in intervals:
        ...
        dxpy.download_dxfile(interval_file.get_id(), "intervals.interval_list")
        ...
        print("Performing coverage calculation for interval : " + interval_type)
    ...
    output_txt = 'sample_%s.coverage.txt' % sample_id
    output_gzip = output_txt + '.gz'

    with open(output_txt, "w") as fh:
        header = list(cov_values.keys())
        output_writer = csv.DictWriter(fh, delimiter="\t", lineterminator="\n", fieldnames=header)
        output_writer.writeheader()
        output_writer.writerow(cov_values)
        fh.close()
    ...
    print("Commands finished... writing output...")
